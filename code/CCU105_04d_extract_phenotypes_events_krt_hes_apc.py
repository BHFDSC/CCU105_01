...
display(codelist_krt.groupBy("phenotype", "terminology").agg(f.count("*")).orderBy(f.col("phenotype")))

# COMMAND ----------

# Keep diagnostic ICD10 codes
diag_codelist = codelist_krt.filter(f.col("terminology")=="icd10").withColumn("code", f.lower(f.col("code"))).withColumn("code", f.regexp_replace(f.col("code"), r"\.", ""))
...
save_table(hes_apc_pheno_sel, f'ccu105_phenotypes_krt_hes_apc_all_events', save_previous=False)

# COMMAND ----------

hes_apc_pheno_sel_relaod = spark.table(f'{proj_params.dbc}.ccu105_phenotypes_krt_hes_apc_all_events')
